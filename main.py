...
parser.add_argument(
    "--camera", type=int, default=None, help="Camera port, if not using JeVois."
)
parser.add_argument(
    "--match-length",
    type=int,
    default=180,
    help="Match length in seconds. Defaults to 180 seconds.",
)
parser.add_argument(
    "--puck_tracking", action="store_true", help="Enable puck tracking."
)
parser.add_argument(
    "--config",
    type=str,
    default="config.json",
    help="ArUco config .json file. Defaults to config.json.",
)
parser.add_argument("--debug", action="store_true", help="Enable debug logging.")
parser.add_argument(
    "--debug-info", action="store_true", help="Enable debug logging at info level."
)
parser.add_argument(
    "--radio_port", type=str, default=None, help="Radio port (i.e., if using Zigbee)."
)
...
rob_track = RobotTracker(aruco_config=args.config).start()
if args.puck_tracking:
    ...
    puck_track = None
if args.radio_port is not None:
    broadcaster = XBeeBroadcaster(port=args.radio_port)
else:
    broadcaster = None
timer = PausableTimer()
gm = GameManager(
